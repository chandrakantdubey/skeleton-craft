...
const ExportModal: React.FC<ExportModalProps> = ({ elements, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50" onClick={onClose}>
      <div 
        className="w-full max-w-3xl h-[80vh] bg-slate-800 rounded-xl shadow-2xl flex flex-col overflow-hidden ring-1 ring-slate-700"
        onClick={e => e.stopPropagation()}
      >
        <header className="flex items-center justify-between p-4 border-b border-slate-700/50 flex-shrink-0">
          <h2 className="text-lg font-semibold text-slate-200">Export Code</h2>
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
            <nav className="flex-shrink-0 md:w-40 p-2 md:p-4 border-b md:border-b-0 md:border-r border-slate-700/50">
                <ul className="flex flex-row md:flex-col gap-1">
                    {EXPORT_FORMATS.map(format => (
                        <li key={format}>
                            <button 
                                onClick={() => setActiveFormat(format)}
                                className={`w-full text-left text-sm font-medium px-3 py-2 rounded-md transition-colors ${activeFormat === format ? 'bg-indigo-600 text-white' : 'text-slate-300 hover:bg-slate-700/50'}`}
                            >
                                {format}
                            </button>
                        </li>
